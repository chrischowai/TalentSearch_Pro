...
export const CandidateScatterPlot: React.FC<CandidateScatterPlotProps> = ({ 
  ...
  const processedData = React.useMemo(() => data.map((candidate, index) => ({
    ...candidate,
    years_experience: Math.round(candidate.years_experience + (index % 3 - 1) * 0.1),
    fitting_score: Math.round(candidate.fitting_score + (index % 5 - 2) * 0.1),
    // Store original values for tooltip
    original_years_experience: Math.round(candidate.years_experience),
    original_fitting_score: Math.round(candidate.fitting_score),
  })), [data]);
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <XAxis
              ...
              label={{ value: 'Years of Experience', position: 'insideBottom', offset: -5, style: { textAnchor: 'middle', fontSize: 12, fill: '#666', fontWeight: 'bold' } }}
            />
            <YAxis
              type="number"
